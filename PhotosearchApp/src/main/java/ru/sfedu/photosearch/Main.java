        ...
        switch (args.get(1)) {
            ...
            case Constants.M_ADD_COMMENT: {
                ...
                if (user.isPresent() && photo.isPresent()){
                    try {
                        Integer rate = Integer.parseInt(args.get(5));
                        if ((rate < 6) && (rate >= 0)) {
                            ...
                            log.error(Constants.ERROR_RATE_LIMIT);
                            return false;
                        }
                    } catch (Exception ex) {
                        log.error(String.format(Constants.ERROR_PARSE_RATE, args.get(5)) + ex);
                        return false;
                    }
            ...
            case Constants.M_ADD_FEEDBACK: {
                ...
                if (user.isPresent() && photographer.isPresent()){
                    try {
                        ...
                        log.error(String.format(Constants.ERROR_PARSE_RATE, args.get(4)) + ex);
                        return false;
                    }
                } else {
                    log.error(Constants.FAILURE + args.get(1));
